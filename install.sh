...
main() {
  ...
  local method="${SS_METHOD:-aes-128-gcm}"
  ...
  local unit_path="/etc/systemd/system/shadowsocks-server.service"
  write_systemd_unit "$unit_path" "$ss_bin" "$config_path" "$config_dir" "$user"

  systemctl daemon-reload
  systemctl enable --now shadowsocks-server.service >/dev/null

  if ! systemctl is-active --quiet shadowsocks-server.service; then
    log_warn "服务启动失败：shadowsocks-server.service"
